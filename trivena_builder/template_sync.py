...
from trivena_framework.utils import get_url, now

from trivena_builder.export_import_standard_page import extract_fonts_from_blocks, import_fonts
...
def safe_segment(name):
	...
	if segment in ("", ".", ".."):
		trivena.throw(trivena._("Unsafe template fixture name: {0}").format(name))
	return segment


def get_templates_root(app="trivena_builder"):
	return os.path.join(trivena.get_app_path(app), "builder_templates")


def get_group_assets_root(group_folder, app="trivena_builder"):
	return os.path.join(trivena.get_app_path(app), "www", "builder_assets", group_folder)


# ---------------------------------------------------------------------------
# import
# ---------------------------------------------------------------------------
def sync_builder_templates(app="trivena_builder", publish=False):
	...
	for group in sorted(os.listdir(templates_root)):
		...
		try:
			...
			trivena.log_error(title=f"Failed to sync template group {group}")
...
def import_template_pages(pages_path, group, publish=False):
	...
	for fname in sorted(os.listdir(pages_path)):
		...
		with open(page_file, encoding="utf-8") as f:  # nosemgrep
			fixture = trivena.parse_json(f.read())
		page_name = fixture.get("name") or fname
		import_file_by_path(page_file)
		if trivena.db.exists("Builder Page", page_name):
			...
			trivena.db.set_value("Builder Page", page_name, values, update_modified=False)
...
def reconcile_deleted_templates(groups_pages):
	...
	template_pages = trivena.get_all(
		"Builder Page",
	...
	for page in template_pages:
		...
		if page.name not in groups_pages[page.template_group]:
			trivena.delete_doc("Builder Page", page.name, force=True, ignore_permissions=True)


# ---------------------------------------------------------------------------
# export (developer mode)
# ---------------------------------------------------------------------------
def export_template_group(group, target_app="trivena_builder"):
	...
	group_folder = safe_segment(trivena.scrub(group))
	...
	for page_name in pages:
		page_doc = trivena.get_doc("Builder Page", page_name)
...
def export_template_page(page_doc, pages_path, group_folder, target_app="trivena_builder"):
	...
	blocks = trivena.parse_json(page_config.get("draft_blocks") or page_config.get("blocks") or "[]")
	...
	export_name = safe_segment(trivena.scrub(str(page_doc.name)))
	...
	with open(os.path.join(page_dir, f"{export_name}.json"), "w", encoding="utf-8") as f:  # nosemgrep
		f.write(trivena.as_json(page_config, ensure_ascii=False))
	return blocks


def export_template_component(component_id, components_path, group_folder, target_app="trivena_builder"):
	...
	component_config["block"] = trivena.as_json(component_blocks, indent=0)
	# page references are site-local and meaningless in fixtures
	component_config["for_web_page"] = None

	export_name = safe_segment(trivena.scrub(str(component_doc.name)))
	...
	with open(os.path.join(component_dir, f"{export_name}.json"), "w", encoding="utf-8") as f:  # nosemgrep
		f.write(trivena.as_json(component_config, ensure_ascii=False))
...
def export_template_variables(group, variables_path):
	...
	for var in trivena.get_all(
		...
		with open(os.path.join(var_dir, f"{safe_name}.json"), "w", encoding="utf-8") as f:  # nosemgrep
			f.write(trivena.as_json(var_config, ensure_ascii=False))


def export_template_fonts(fonts, fonts_path, group_folder, target_app="trivena_builder"):
	...
	for font_name in fonts:
		...
		export_name = safe_segment(trivena.scrub(font.font_name))
		...
		with open(os.path.join(font_dir, f"{export_name}.json"), "w", encoding="utf-8") as f:  # nosemgrep
			f.write(trivena.as_json(font_config, ensure_ascii=False))


def ensure_template_preview(page_doc, app="trivena_builder"):
	...
	from trivena_builder.utils import get_builder_page_preview_file_paths
	...
	if not os.path.exists(local_path):
		try:
			...
			trivena.log_error(f"Failed to generate preview for template page {page_doc.name}")
...
def update_template_manifest(group_path, page_names, title=None):
	...
	with open(manifest_path, "w", encoding="utf-8") as f:  # nosemgrep
		f.write(trivena.as_json(manifest, ensure_ascii=False))


def delete_template_page_fixture(page_doc, app="trivena_builder"):
	...
	group_folder = safe_segment(trivena.scrub(str(page_doc.template_group)))
	group_path = os.path.join(get_templates_root(app), group_folder)

	page_segment = safe_segment(trivena.scrub(str(page_doc.name)))
	...
	if os.path.isdir(pages_path):
		...
		for fname in sorted(os.listdir(pages_path)):
			...
			if os.path.isfile(page_file):
				with open(page_file, encoding="utf-8") as f:  # nosemgrep
					remaining.append(trivena.parse_json(f.read()).get("name") or fname)
...
# ---------------------------------------------------------------------------
def get_group_manifest(group_folder, app="trivena_builder"):
...
def get_all_group_manifests(app="trivena_builder"):
...
def copy_block_assets(blocks, group_folder, subpath, app="trivena_builder"):
...
def copy_file_url_to_group_assets(file_url, group_folder, subpath, app="trivena_builder"):
	...
	file_name = trivena.db.get_value("File", {"file_url": file_url}, "name")
	if not file_name:
		return None

	source_path = trivena.get_doc("File", file_name).get_full_path()
	return copy_file_to_group_assets(source_path, group_folder, subpath, os.path.basename(file_url), app=app)


def copy_file_to_group_assets(source_path, group_folder, subpath, filename=None, app="trivena_builder"):
...
def resolve_asset_source_path(file_url, app="trivena_builder"):
	"""Resolve a /files/* or /builder_assets/* url to a local filesystem path."""
	if not file_url or not isinstance(file_url, str):
		return None
	if file_url.startswith("/files/"):
		path = os.path.join(trivena.local.site_path, "public", file_url.lstrip("/"))
	elif file_url.startswith("/builder_assets/"):
		path = os.path.join(trivena.get_app_path(app), "www", file_url.lstrip("/"))
	else:
		return None
