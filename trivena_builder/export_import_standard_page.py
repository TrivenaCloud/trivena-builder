...
def export_page_as_standard(page_name, target_app):
	...
	export_name = trivena.scrub(page_doc.page_name)

	app_path = trivena.get_app_path(target_app)
	if not app_path:
		trivena.throw(f"App '{target_app}' not found")
	...
	blocks = trivena.parse_json(page_config.get("draft_blocks") or page_config["blocks"])
	if blocks:
	...
	page_config = trivena.as_json(page_config, ensure_ascii=False)
	...
	if blocks:
		...
		for component_id in components:
			try:
				component_doc = trivena.get_cached_doc("Builder Component", component_id)
...
def sync_standard_builder_pages(app_name=None):
	...
	apps_to_sync = [app_name] if app_name else trivena.get_installed_apps()

	for app in apps_to_sync:
		app_path = trivena.get_app_path(app)
...
def export_fonts(fonts, builder_files_path, assets_path, target_app="trivena_builder"):
	...
	for font_name in fonts:
		try:
			...
			safe_font_name = trivena.scrub(font_name)
			...
			trivena.log_error(f"Failed to export font {font_name}: {e!s}")


def copy_font_file(file_url, assets_path, target_app="trivena_builder"):
	...
	try:
		...
		trivena.log_error(f"Failed to copy font file {file_url}: {e!s}")
...
def export_variables(variables, builder_files_path):
	...
	for var_name in variables:
		try:
			...
			safe_var_name = trivena.scrub(var_doc.variable_name)
			...
			trivena.log_error(f"Failed to export variable {var_name}: {e!s}")
			...
			if os.path.exists(font_file):
				try:
					with open(font_file) as f:
						font_config = trivena.parse_json(f.read())

					if trivena.db.exists("User Font", font_config.get("font_name")):
						continue

					font_doc = trivena.get_doc(
						{
							"doctype": "User Font",
					...
					trivena.log_error(f"Failed to import font {fname}: {e!s}")
