...
from trivena_framework.desk.form.load import getdoc
from trivena_framework.tests.utils import FrappeTestCase
from trivena_framework.website.serve import get_response, get_response_content

from trivena_builder.builder.component_versions import ensure_component_version
from trivena_builder.utils import Block
...
class TestBuilderPage(FrappeTestCase):
	...
	def setUpClass(cls):
		...
		cls.page_with_dynamic_route = trivena.get_doc(
			{
				"doctype": "Builder Page",
	...
	def test_get_preview_html_preserves_outer_request(self):
		...
		from trivena_framework.utils import CallbackManager
		from werkzeug.test import EnvironBuilder
		from werkzeug.wrappers import Request

		previous = getattr(trivena.local, "request", None)
		try:
			req = Request(EnvironBuilder(path="/api/method/run_doc_method").get_environ())
			sentinel = CallbackManager()
			req.after_response = sentinel
			trivena.local.request = req

			self.page.get_preview_html()

			self.assertIs(trivena.local.request, req)
			self.assertIs(trivena.local.request.after_response, sentinel)
		finally:
			trivena.local.request = previous

	def test_onload(self):
		getdoc("Builder Page", self.page.name)
		self.assertEqual(trivena.response.docs[0].get("__onload").get("builder_path"), "trivena_builder")

	def test_dynamic_route(self):
		from trivena_framework.utils import get_html_for_route
	...
	def test_publish_unpublish(self):
		...
		self.assertNotIn("Hello World!", trivena.safe_decode(response.get_data()))
	...
	def test_client_script(self):
		client_script_js = trivena.get_doc(
			{
		...
		client_script_css = trivena.get_doc(
			{
				"doctype": "Builder Client Script",
				"script_type": "CSS",
		...
		client_script_js_row = trivena.get_doc(
			{
		...
		client_script_css_row = trivena.get_doc(
			{
				"doctype": "Builder Page Client Script",
				"parent": page.name,
	...
	def test_duplicate_binding_deduped_in_placeholders(self):
		...
		from trivena_builder.builder.doctype.builder_page.builder_page import set_dynamic_content_placeholders
	...
	def test_component_dynamic_values(self):
		...
		page_with_component_no_overrides = trivena.get_doc(
			{
				"doctype": "Builder Page",
		...
		page_with_component_having_overrides = trivena.get_doc(
			{
				"doctype": "Builder Page",
				"page_title": "Component Dynamic Values Test",
				"published": 1,
		...
		page_with_component_having_bad_overrides = trivena.get_doc(
			{
				"doctype": "Builder Page",
				"page_title": "Component Dynamic Values Test",
				"published": 1,
	...
	def test_redirect_from_page_data_script(self):
		...
		try:
			...
			trivena.local.flags.redirect_location = None
			page.delete()
...
"""
		...
		try:
			...
			self.assertNotIn("/assets/trivena_builder/js/reactivity.js", content)
			self.assertRegex(
				content,
	...
	def test_component_data_uses_root_prop_defaults(self):
		from trivena_builder.builder.doctype.builder_component.builder_component import get_component_data
	...
	def test_pinned_component_version_keeps_block_client_script(self):
		...
		trivena.clear_document_cache("Builder Component", component.name)
	...
	def test_std_props(self):
		...
		page_with_default_values = trivena.get_doc(
			{
				"doctype": "Builder Page",
		...
		page_with_overridden_values = trivena.get_doc(
			{
				"doctype": "Builder Page",
	...
	def test_set_fonts(self):
		from trivena_builder.builder.doctype.builder_page.builder_page import set_fonts
	...
	def test_get_google_font_urls(self):
		from trivena_builder.builder.doctype.builder_page.builder_page import get_google_font_urls
	...
	def test_italics_cascade_like_font_family(self):
		...
		from trivena_builder.builder.doctype.builder_page.builder_page import get_block_html
	...
	def test_set_italics_from_html(self):
		...
		from trivena_builder.builder.doctype.builder_page.builder_page import set_italics_from_html
	...
	def test_renders_blocks_with_stripped_empty_values(self):
		...
		# CSS class names are a random hash per render (trivena.generate_hash) — ignore them.
		def normalize(text):
	...
	def test_renders_legacy_raw_styles_from_component(self):
		...
		component = trivena.get_doc(
			{"doctype": "Builder Component", "block": trivena.as_json(component_root)}
		).insert()
	...
	def test_conflicting_routes_picks_last_published(self):
		...
		from trivena_framework.utils import add_to_date, now_datetime
		from trivena_framework.website.utils import clear_cache as clear_page_cache

		from trivena_builder.builder.doctype.builder_page.builder_page import find_page_with_path

		# Frappe strips leading slashes from routes during validation; use without slash
		route = "conflicting-route-test"

		page_older = trivena.get_doc(
			{
				"doctype": "Builder Page",
