...
def execute():
	...
	web_pages = trivena.get_all("Builder Page", fields=["name", "blocks"])
	for web_page in web_pages:
		blocks = trivena.parse_json(web_page.blocks)
		if blocks:
		...
		draft_blocks = trivena.parse_json(web_page.draft_blocks)
		if draft_blocks:
...
def update_blocks(blocks):
	for block in blocks:
		...
		if block.get("extendedFromComponent"):
			try:
				component = trivena.get_cached_doc("Builder Component", block.get("extendedFromComponent"))
				...
				trivena.log_error(f"Builder Component {block.get('extendedFromComponent')} not found")
...
def generate_id():
	return trivena.generate_hash("", 10)
