...
# <script defer data-api="https://frappecloud.com/api/event" data-domain="trivena_framework.io" src="https://frappecloud.com/js/script.js"></script>
...
def execute():
	...
	for page in pages:
		...
		blocks = trivena.parse_json(page.get("blocks"))
		if blocks:
			...
			trivena.db.set_value(
				...
				page.name,
				"blocks",
				trivena.as_json(blocks, indent=None),
				update_modified=False,
			)

		draft_blocks = trivena.parse_json(page.get("draft_blocks"))
		if draft_blocks:
			...
			trivena.db.set_value(
				...
				page.name,
				"draft_blocks",
				trivena.as_json(draft_blocks, indent=None),
				update_modified=False,
			)
	...
	components = trivena.get_all("Builder Component", fields=["name", "block"])
	for component in components:
		component_block = trivena.parse_json(component.get("block"))
		if component_block:
			...
			trivena.db.set_value(
				...
				component.name,
				"block",
				trivena.as_json(component_block, indent=None),
				update_modified=False,
			)
