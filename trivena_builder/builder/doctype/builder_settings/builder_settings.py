...
from trivena_builder.utils import has_page_read, has_page_write
...
class BuilderSettings(Document):
	...
	def on_update(self):
		...
		if self.has_value_changed("home_page"):
			trivena.cache.delete_key("home_page")
	...
	def update_script_file(self, attribute, script_type, extension, folder_name):
		...
		public_url = f"/files/{folder_name}/{file_name}?v={trivena.generate_hash(length=10)}"
...
def get_website_user_home_page(session_user=None):
	home_page = trivena.get_cached_value("Builder Settings", None, "home_page")
	return home_page
...
def get_components():
	...
	return trivena.get_all("Builder Component", fields=["name as value", "component_name as label"])
...
def replace_component(target_component: str, replace_with: str, filters: str | None = None):
	...
	if not trivena.db.exists("Builder Component", replace_with):
		trivena.throw(_("The component you are trying to replace with does not exist"))

	# go through all the pages and replace the old component with the new component
	pages = trivena.get_all(
		"Builder Page",
		fields=["name"],
		filters=trivena.parse_json(filters) if filters else {},
		or_filters={
	...
	for page in pages:
		doc = trivena.get_doc("Builder Page", page.name)
...
def get_component_usage_count(component_id: str, filters: str | None = None):
	if not trivena.has_permission("Builder Page", ptype="read"):
