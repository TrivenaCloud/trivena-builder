...
def take_snapshot(reference_doctype, reference_name, fields, label=None, snapshot_type=None, transform=None):
	...
	"""
	doc = trivena.get_doc(reference_doctype, reference_name)
	data = {field: doc.get(field) for field in fields}
	if transform:
		data = transform(data)
	snapshot = trivena.get_doc(
		{
...
def get_snapshot_data(snapshot_name) -> dict:
	...
	snapshot = trivena.get_doc("Builder Snapshot", snapshot_name)
	return trivena.parse_json(snapshot.data)
...
def get_versioned_doc(snapshot_name):
	...
	snapshot = trivena.get_doc("Builder Snapshot", snapshot_name)
	try:
		doc = trivena.get_doc(snapshot.reference_doctype, snapshot.reference_name)
	except trivena.DoesNotExistError:
		doc = trivena.new_doc(snapshot.reference_doctype)
		doc.name = snapshot.reference_name
	for field, value in trivena.parse_json(snapshot.data).items():
		doc.set(field, value)
	return doc
...
def prune_snapshots(reference_doctype, reference_name, keep, snapshot_type=None):
	...
	for name in names[keep:]:
		trivena.delete_doc("Builder Snapshot", name, ignore_permissions=True)
