...
from trivena_framework.modules.export_file import delete_folder, export_to_files
from trivena_framework.utils.caching import redis_cache
from trivena_framework.website.utils import delete_page_cache
...
class BuilderVariable(Document):
	...
	def on_update(self):
		...
		if self.is_standard:
			export_to_files(
				record_list=[["Builder Variable", self.name, "builder_variable"]], record_module="trivena_builder"
			)

		if self.has_value_changed("is_standard") and not self.is_standard:
			delete_folder("trivena_builder", "builder_variable", self.name)
...
def get_css_variables():
	builder_variables = trivena.get_all("Builder Variable", fields=["name", "value", "dark_value"])
	css_variables = {}
