...
from trivena_framework.website.utils import clear_website_cache

from trivena_builder.builder.component_versions import ensure_component_version
from trivena_builder.utils import Block, compact_json, execute_script
...
class BuilderComponent(Document):
	...
	def before_insert(self):
		if not self.component_id:
			self.component_id = trivena.generate_hash(length=16)
		capture("builder_component_created", "trivena_builder")
	...
	def clear_page_cache(self):
		pages = trivena.get_all("Builder Page", filters={"published": 1}, fields=["name"])
		for page in pages:
			page_doc = trivena.get_cached_doc("Builder Page", page.name)
	...
	def update_exported_component(self):
		...
		component_path = os.path.join(
			trivena.get_app_path("trivena_builder"), "trivena_builder", "builder_component", self.name
		)
...
class ComponentSyncer:
	...
	def sync_blocks(self, blocks: str | list[Block], component) -> str:
		...
		for block in blocks_list:
			...
			if block.extendedFromComponent == component.component_id:
				component_block = Block(**trivena.parse_json(component.block))
	...
	def parse_blocks(blocks: str) -> list[Block]:
		...
		blocks_list = trivena.parse_json(blocks)
...
def get_component_data(
	...
	try:
		component_doc = trivena.get_cached_doc("Builder Component", component_name)
	...
	if isinstance(props, str):
		props = trivena.parse_json(props)

	if not script:
		return {}

	_locals = dict(
		component=trivena._dict(),
		props=trivena._dict(props or {}),
	)
...
def get_component_prop_values(block: dict | str | None) -> dict:
	block = trivena.parse_json(block or "{}")
	return {
