...
from trivena_builder.utils import compact_json
...
def pin_components_in_page_data(data: dict) -> dict:
	...
	for key in ("draft_blocks", "blocks"):
		...
		if not value:
			continue
		blocks = copy.deepcopy(trivena.parse_json(value))
...
def canonical(block_json, strip_pins=False):
	...
	parsed = trivena.parse_json(block_json or "{}")
	if strip_pins:
		walk_blocks(parsed, lambda block: block.pop("componentVersion", None))
	return trivena.as_json(parsed)
...
def ensure_component_version(
	...
	values = trivena.db.get_value("Builder Component", component_id, COMPONENT_VERSION_FIELDS, as_dict=True)
	if values is None:
	...
	try:
		block = trivena.parse_json(values.block or "{}")
...
def resolve_component(component_id: str, pinned_version: str | None = None) -> dict | None:
	...
	if pinned_version:
		...
		if data:
			return trivena.parse_json(data)
	values = trivena.get_cached_value(
...
def canonical_component(data: dict) -> str:
	...
	block = out.get("block")
	if block:
		parsed = trivena.parse_json(block)
		walk_blocks(parsed, lambda b: b.pop("componentVersion", None))
		out["block"] = trivena.as_json(parsed)
	...
	def check(block):
		...
		if component_id and component_id not in seen:
			...
			if resolve_component(component_id, block.get("componentVersion")) is None:
				warnings.append(
					trivena._("Component {0} was deleted and will not render").format(component_id)
				)

	walk_blocks(trivena.parse_json(blocks_json or "[]"), check)
	return warnings
