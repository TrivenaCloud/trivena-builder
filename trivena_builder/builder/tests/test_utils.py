...
class TestBuilderUtils(FrappeTestCase):
	...
	@patch("trivena_framework.utils.safe_exec.is_safe_exec_enabled", return_value=False)
	def test_execute_script_with_enabled_server_script(self, *args):
		script = "data.test = trivena.get_doc('User', 'Administrator').email"
		_locals = dict(data=trivena._dict())
	...
	@patch("trivena_framework.utils.safe_exec.is_safe_exec_enabled", return_value=True)
	def test_execute_script_with_disabled_server_script(self, *args):
		...
		script = "data.users = trivena.db.get_all('User')"
		execute_script(script, _locals, "test.py")
	...
	def test_extract_components_from_blocks(self):
		...
		# Mock trivena.get_cached_doc using unittest.mock

		with patch("trivena_framework.get_cached_doc") as mock_get_cached_doc:
			mock_get_cached_doc.return_value = trivena._dict(block='{"element": "div"}')
	...
	def test_copy_img_to_asset_folder(self):
		test_page = trivena._dict(name="test-page")
		...
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict()
		block.children = []
		copy_img_to_asset_folder(block, test_page)  # Should not raise error
		...
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict(src="https://example.com/image.jpg")
		block.children = []
		...
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict(src="/builder_assets/local-image.jpg")
		block.children = []
		original_src = block.attributes.src
		copy_img_to_asset_folder(block, test_page)
		self.assertEqual(block.attributes.src, original_src)  # Should remain unchanged
		...
		child_block.attributes = trivena._dict(src="https://example.com/nested.jpg")
		child_block.children = []

		parent_block = Block()
		parent_block.element = "div"
		parent_block.attributes = trivena._dict()
		...
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict(src="/files/nonexistent-image.jpg")
		block.children = []

		with patch("trivena_framework.get_all") as mock_get_all:
		...
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict(src="/files/test-image.jpg")
		block.children = []

		mock_file = trivena._dict()
		mock_file.get_full_path = lambda: "/fake/path/test-image.jpg"

		with (
			patch("trivena_framework.get_all") as mock_get_all,
			patch("trivena_framework.get_doc") as mock_get_doc,
			patch("shutil.copy") as mock_copy,
			patch("builder.utils.get_template_assets_folder_path") as mock_get_path,
		):
			mock_get_all.return_value = [trivena._dict(name="file-123")]
		...
		site_url = trivena.utils.get_url()
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict(src=f"{site_url}/files/prefixed-image.jpg")
		block.children = []
		...
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict(src="/files/image%20with%20spaces.jpg")
		block.children = []
		...
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict(src="/files/error-test.jpg")
		block.children = []
		...
		with (
			...
			mock_get_all.return_value = [trivena._dict(name="file-456")]
			mock_get_doc.return_value = mock_file
			mock_get_path.return_value = "/fake/assets/path"
		...
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict(src="")
		block.children = []
		copy_img_to_asset_folder(block, test_page)  # Should not process empty src

		block.attributes = trivena._dict(src=None)
		copy_img_to_asset_folder(block, test_page)  # Should not process None src

		# Test with malformed URLs
		block = Block()
		block.element = "img"
		block.attributes = trivena._dict(src="/uploads/image.jpg")  # Not a /files path
		block.children = []
		original_src = block.attributes.src
		copy_img_to_asset_folder(block, test_page)
		self.assertEqual(block.attributes.src, original_src)  # Should remain unchanged
		...
		grandchild.attributes = trivena._dict(src="/files/deep-nested.jpg")
		grandchild.children = []

		child = Block()
		child.element = "span"
		child.attributes = trivena._dict()
		child.children = [grandchild]

		parent = Block()
		parent.element = "div"
		parent.attributes = trivena._dict()
