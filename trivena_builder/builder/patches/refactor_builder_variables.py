...
from trivena_builder.builder.doctype.builder_variable.builder_variable import get_css_variables
from trivena_builder.utils import camel_case_to_kebab_case
...
def execute():
	...
	"""
	if not trivena.db.has_column("Builder Variable", "variable_name"):
		return

	variables = trivena.get_all("Builder Variable", fields=["name", "variable_name"])
...
def rename_variables(rename_map):
	...
	for old_name, new_id in rename_map.items():
		try:
			trivena.rename_doc("Builder Variable", old_name, new_id, force=True, merge=False)
			renamed += 1
...
def normalise_type_spacing():
	trivena.db.sql("UPDATE `tabBuilder Variable` SET type='Dimension' WHERE type='Spacing'")
...
def rewrite_doctype_blocks(doctype, fields, css_rewrite_map):
	...
	updated = 0
	for rec in trivena.get_all(doctype, fields=["name", *fields]):
		...
		if dirty:
			for f, val in dirty.items():
				trivena.db.set_value(doctype, rec.name, f, val, update_modified=False)
			updated += 1
	return updated
