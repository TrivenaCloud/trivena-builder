...
from trivena_framework.modules.import_file import import_file_by_path
from trivena_framework.utils import get_url
from trivena_framework.utils.safe_exec import (
...
def compact_json(obj) -> str:
	return trivena.as_json(obj, indent=None, separators=(",", ":"))
...
def has_page_permission(ptype: str = "write", message: str | None = None):
	...
	def decorator(fn):
		...
		def wrapper(*args, **kwargs):
			if not trivena.has_permission("Builder Page", ptype=ptype):
				default_message = (
					trivena._("You do not have permission to modify pages")
					if ptype == "write"
					else trivena._("You do not have permission to read pages")
				)
				trivena.throw(message or default_message)
...
class Block:
	...
	def as_json(self, wrap_in_array=False):
		return trivena.as_json([self.as_dict()]) if wrap_in_array else trivena.as_json(self.as_dict())
...
def get_cached_doc_as_dict(doctype, name):
	assert isinstance(doctype, str)
	assert isinstance(name, str)
	return trivena.get_cached_doc(doctype, name).as_dict()
...
def make_safe_get_request(url, **kwargs):
	...
	return trivena.integrations.utils.make_get_request(url, **kwargs)
...
def get_safer_globals():
	...
	form_dict = getattr(trivena.local, "form_dict", trivena._dict())

	if "_" in form_dict:
		del trivena.local.form_dict["_"]

	out = NamespaceDict(
		json=safe_globals["json"],
		as_json=trivena.as_json,
		dict=safe_globals["dict"],
		args=form_dict,
		frappe=NamespaceDict(
			db=NamespaceDict(
				count=trivena.db.count,
				exists=trivena.db.exists,
				get_all=safe_get_all,
				get_list=safe_get_list,
				get_single_value=trivena.db.get_single_value,
			),
			form_dict=form_dict,
...
def safer_exec(
	...
	*,
	...
	if script_filename:
		filename += f": {trivena.scrub(script_filename)}"
...
def sync_page_templates():
	...
	builder_component_path = trivena.get_module_path("trivena_builder", "builder_component")
	make_records(builder_component_path)

	print("Syncing Builder Scripts")
	builder_script_path = trivena.get_module_path("trivena_builder", "builder_script")
	make_records(builder_script_path)


def sync_block_templates():
	print("Syncing Builder Block Templates")
	builder_block_template_path = trivena.get_module_path("trivena_builder", "builder_block_template")
	make_records(builder_block_template_path)


def sync_builder_variables():
	print("Syncing Builder Builder Variables")
	builder_variable_path = trivena.get_module_path("trivena_builder", "builder_variable")
...
def copy_img_to_asset_folder(block, page_doc, app=None):
	...
	if isinstance(block, dict):
		block = trivena._dict(block)
		children = block.get("children", [])
		if children and isinstance(children, list):
			block.children = [trivena._dict(child) if isinstance(child, dict) else child for child in children]
	...
	if element == "img":
		...
		if src and (src.startswith(f"{site_url}/files") or src.startswith("/files")):
			...
			src = unquote(src)
			files = trivena.get_all("File", filters={"file_url": src}, fields=["name"])
			if files:
...
def get_template_assets_subfolder(page_doc):
	...
	if getattr(page_doc, "is_template", None) and getattr(page_doc, "template_group", None):
		return os.path.join(trivena.scrub(page_doc.template_group), str(page_doc.name))
...
def template_target_app():
	...
	return trivena.conf.get("template_target_app") or "trivena_builder"


def get_template_assets_folder_path(page_doc, app=None):
	path = os.path.join(
		trivena.get_app_path(app or template_target_app()),
		"www",
		"builder_assets",
...
def get_builder_page_preview_file_paths(page_doc, app=None):
	...
	if page_doc.is_template:
		...
		local_path = os.path.join(trivena.local.site_path, "public", "files", file_name)
		random_hash = trivena.generate_hash(length=5)
...
def add_composite_index_to_web_page_view():
	...
	"""
	trivena.db.add_index("Web Page View", ["creation", "is_unique", "path"])
...
def copy_assets_from_blocks(blocks, assets_path, target_app="trivena_builder"):
	if not isinstance(blocks, list):
		blocks = [blocks]
...
def process_block_assets(block, assets_path, target_app="trivena_builder"):
...
def copy_asset_file(file_url, assets_path, target_app="trivena_builder"):
	...
	try:
		...
		trivena.log_error(f"Failed to copy asset {file_url}: {e!s}")
	return None


def copy_from_site_files(file_url, assets_path, target_app="trivena_builder"):
	"""Copy file from site files directory"""
	source_path = os.path.join(trivena.local.site_path, "public", file_url.lstrip("/"))
	if os.path.exists(source_path):
		return copy_file_to_assets(source_path, file_url, assets_path, target_app)
	return None


def copy_from_builder_assets(file_url, assets_path, target_app="trivena_builder"):
	"""Copy file from trivena_builder assets directory"""
	if file_url.startswith("/assets/") and "/builder_assets/" in file_url:
		parts = file_url.split("/")
		if len(parts) >= 3:
			app_name = parts[2]
			source_path = os.path.join(trivena.get_app_path(app_name), "public", "/".join(parts[3:]))
	else:
		source_path = os.path.join(trivena.get_app_path("trivena_builder"), "www", file_url.lstrip("/"))
	if os.path.exists(source_path):
		return copy_file_to_assets(source_path, file_url, assets_path, target_app)
	return None


def copy_file_to_assets(source_path, file_url, assets_path, target_app="trivena_builder"):
	...
	for block in blocks:
		if isinstance(block, dict):
			if block.get("extendedFromComponent"):
				component_doc = trivena.get_cached_doc("Builder Component", block["extendedFromComponent"])
				if component_doc:
					components.update(
						extract_components_from_blocks(trivena.parse_json(component_doc.block or "{}"))
					)
...
def export_client_scripts(page_doc, client_scripts_path):
	...
	from trivena_framework.modules.export_file import strip_default_fields

	for script_row in page_doc.client_scripts:
		script_doc = trivena.get_doc("Builder Client Script", script_row.builder_script)
		script_config = script_doc.as_dict(no_nulls=True)
		script_config = strip_default_fields(script_doc, script_config)
		fname = trivena.scrub(str(script_doc.name))
		...
		with open(script_file_path, "w", encoding="utf-8") as f:
			f.write(trivena.as_json(script_config, ensure_ascii=False))


def export_components(components, components_path, assets_path, target_app="trivena_builder"):
	"""Export components to files"""
	for component_id in components:
		try:
			component_doc = trivena.get_doc("Builder Component", component_id)
			# replace assets in component blocks
			component_blocks = trivena.parse_json(component_doc.block or "[]")
			copy_assets_from_blocks(component_blocks, assets_path, target_app)
			component_doc.block = trivena.as_json(component_blocks)

			# Replace forward slashes with underscores to create valid directory names
			safe_component_name = trivena.scrub(component_doc.component_name).replace("/", "_")
			...
			with open(component_file_path, "w") as f:
				f.write(trivena.as_json(component_doc.as_dict()))
		except Exception as e:
			print(e)
			trivena.log_error(f"Failed to export component {component_id}: {e!s}")
...
def hash(s):
	return f"{trivena.generate_hash(length=6)}-{s}"


def to_safe_json(data):
	return trivena.as_json(data or {}).replace("</", r"<\/")
