...
from trivena_builder.utils import has_page_write, to_compact_yaml
...
def validate_image_data(image_data: str) -> str:
	...
	if not image_data.startswith("data:image/"):
		trivena.throw(_("Invalid image data: must be a base64-encoded image data URL"))
	if ";base64," not in image_data:
		trivena.throw(_("Invalid image data: must be a base64-encoded data URL"))
	# ~5 MB image ≈ ~6.7 MB base64 string
	if len(image_data) > 7 * 1024 * 1024:
		trivena.throw(_("Image is too large. Please use an image smaller than 5 MB."))
	return image_data
...
def run_llm_job(
	...
	def emit(suffix, **kwargs):
		...
		trivena.publish_realtime(event, payload, user=user)
	...
	if cache_key:
		trivena.cache().set_value(cache_key, {"content": "", "task_type": task_type}, expires_in_sec=600)
	...
	try:
		...
		if cache_key:
			trivena.cache().delete_value(cache_key)
		trivena.log_error(f"Parse error: {e}\nContent: {content}", f"{event_prefix} parse")
		...
		if cache_key:
			trivena.cache().delete_value(cache_key)
		trivena.log_error(f"LLM job error: {e}", event_prefix)
...
def enqueue_ai_job(fn, model=None, **kwargs):
	if not trivena.has_permission("Builder Page", ptype="write"):
		trivena.throw(_("You do not have permission to modify pages"))
	settings = trivena.get_single("Builder Settings")
	...
	if not api_key:
		trivena.throw(_("Please configure an OpenRouter API key in Settings → AI"))
	...
	capture("builder_ai_used", "trivena_builder")
	trivena.local.response.http_status_code = 202
...
def modify_section_from_prompt(
	...
	try:
		...
		trivena.throw(_("Invalid block context JSON"))
	image_url = validate_image_data(image_data) if image_data else None
	return enqueue_ai_job(
...
def get_ai_streaming_content(page_id: str):
	...
	return trivena.cache().get_value(cache_key) or {"content": None}
