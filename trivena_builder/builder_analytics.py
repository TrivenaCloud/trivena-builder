...
class DuckDBConnection:
	...
	def __enter__(self):
		duckdb_path = os.path.join(trivena.get_site_path(), "builder_analytics.duckdb")
...
def setup_table(table_name: str, doctype: str, fields: list[str]):
	...
	with DuckDBConnection() as db:
		df = pd.read_sql(f"SELECT {', '.join(fields)} FROM `tab{doctype}`", trivena.db.get_connection())  # type: ignore
		db.register("df", df)
		...
		trivena.logger().info(f"Ingested {len(df)} {doctype} records into DuckDB ({table_name})")
...
def get_page_analytics(
	...
	try:
		...
		trivena.log_error("DuckDB Analytics Error", str(e))
		return get_empty_analytics()
...
def get_top_pages(
	...
	try:
		...
		trivena.log_error("DuckDB Analytics Error in top pages", str(e))
		return []
...
def get_top_referrers(
	...
	try:
		...
		trivena.log_error("DuckDB Analytics Error in top referrers", str(e))
		return []
...
def get_page_ctr(
	...
	try:
		...
		trivena.log_error("DuckDB CTR Error", str(e))
		return get_empty_ctr()
